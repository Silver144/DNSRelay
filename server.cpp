...
DNSRelay::DNSRelay(int argc, char *argv[])
{

    cli_socket = new QUdpSocket(this);
    sender = new QUdpSocket(this);

    cli_socket->bind(QHostAddress("0.0.0.0"), 53);
	time_init = time(NULL);
	...
	{
		std::cout << "open url_ip_file error\n";
		exit(0);
		/* 调试是否成功读入 */
	}
...
void DNSRelay::handle_param(int argc, char *argv[])
{
	if (argc == 1)
		return;
...
//std::string DNSRelay::addr_transform(char *addr)
//{
//	std::string t_addr;
//	while (*addr)
//	{
...
void DNSRelay::data_read_ready()
{
	//printf("recv packet\n");
	QByteArray datagram;
	datagram.resize(static_cast<int>(cli_socket->pendingDatagramSize()));
	QHostAddress addr;
	quint16 port;
...
void DNSRelay::data_recv_ready()
{
	...
	QHostAddress addr;
	quint16 port;
    datagram.resize(static_cast<int>(sender->pendingDatagramSize()));
    sender->readDatagram(datagram.data(), datagram.size(), &addr, &port);
	printf("recv from %s:%u\n", addr.toString().toStdString().c_str(), port);
	if (datagram.size() == 0)
		return;
    unsigned char *datar = new unsigned char[datagram.size()];
    int it = 0;
    for (auto &c : datagram)
        datar[it++] = static_cast<unsigned char>(c);
	...
	if (t_header.getQR() == 1 && t_header.getQDCOUNT() == 1)
	{
		...
		if (a_header.getRTYPE() == 0x01 && a_header.getANCOUNT() >= 1) 
		{
			...
			ip = QHostAddress(*((int *)(rs_copy))).toString().toStdString();
		...
		if (debug_level == 2)
		{
			...
			if (t_header.getANCOUNT() >= 1)
			{
				...
				if (a_header.getRTYPE() == 0x01)
					printf("resource: %s\n", QHostAddress(*((int *)(rs_copy))).toString().toStdString().c_str());
	...
	unsigned short ID = static_cast<unsigned short>(static_cast<unsigned short>(datar[0]) << 8 | datar[1]);
	unsigned short o_ID = std::get<0>(s_map[ID]);
	addr = std::get<1>(s_map[ID]);
	port = std::get<2>(s_map[ID]);
	datar[0] = static_cast<unsigned char>(o_ID >> 8);
	datar[1] = static_cast<unsigned char>(o_ID);
	QByteArray t_a;
...
std::string DNSRelay::get_cname(std::string resource, quint16 _size, unsigned char *data)
{
	std::string t_s;
	int cnt = 0;
	while (cnt < _size - 1)
	{
		...
		{
			unsigned short us = static_cast<unsigned short>(static_cast<unsigned short>(resource[cnt]) << 8 | resource[cnt + 1]);
			resource = (char *)(data + (us & 0x3fff));
			_size = resource.size(); 
			cnt = 0;
		}
		int clen = resource[cnt];
...
{
    unsigned char *data;
    data = new unsigned char[t_array.size() + 10];
    int it = 0;
    for (auto &c : t_array)
        data[it++] = static_cast<unsigned char>(c);
    ...
    if(!get_ans_from_cache(q_header.QNAME, t_array, addr, port))
    {
		QByteArray t_a;
        /* ID transfrom */
        data[0] = static_cast<unsigned char>(n_count >> 8);
        data[1] = static_cast<unsigned char>(n_count);
        /* insert into ID table */
        s_map[n_count++] = std::tuple<unsigned short, QHostAddress, quint16>(t_header.getID(), addr, port);
        for (int i = 0; i < t_array.size(); i++)
